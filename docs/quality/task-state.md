...
verification:
  ...
  - "Bot Gateway targeted gate: python -m pytest tests/test_bot_gateway.py tests/test_bot_gateway_contracts.py -q -> 37 passed, 6 subtests passed."
  - "Bot Gateway quality gate: python -m ruff check scripts/bot_replies.py scripts/bot_runtime.py scripts/bot_actions.py tests/test_bot_gateway.py -> passed; python -m py_compile scripts/bot_replies.py scripts/bot_runtime.py scripts/bot_actions.py -> passed."
  - "Bot Gateway projection/facade gate: python -m pytest tests/test_bot_gateway.py tests/test_bot_gateway_contracts.py tests/monitor_state/test_projection.py tests/test_dashboard_state_contracts.py -q -> 59 passed, 11 subtests passed."
  - "Bot Gateway full local gates: python -m pytest -q -> 557 passed, 2 skipped, 224 subtests passed; python -m ruff check . -> passed; git diff --check -> passed."
  - "Onboarding targeted gate: python -m pytest tests/tgcs_cli/test_run_demo_init.py tests/tgcs_cli/test_delegates.py tests/test_windows_batch.py tests/test_packaging_metadata.py -q -> 31 passed."
  - "Onboarding quality gate: python -m ruff check scripts/tgcs_launchers.py scripts/tgcs_quickstart.py tests/tgcs_cli/test_run_demo_init.py tests/tgcs_cli/test_delegates.py -> passed; python -m py_compile scripts/tgcs_launchers.py scripts/tgcs_quickstart.py scripts/tgcs.py -> passed."
  - "Onboarding CLI/contract gate: python -m pytest tests/tgcs_cli tests/test_windows_batch.py tests/test_packaging_metadata.py tests/test_agent_native_cli.py tests/test_contract_fixtures.py -q -> 50 passed, 2 subtests passed."
  - "Onboarding full local gates: python -m pytest -q -> 558 passed, 2 skipped, 224 subtests passed; python -m ruff check . -> passed; git diff --check -> passed."
  - "Review undo targeted frontend gate: cd dashboard; npm test -- --run inbox -> 2 files, 17 tests passed."
  ...
  - "Review follow-through targeted frontend gate: cd dashboard; npm test -- --run inbox -> 2 files, 18 tests passed."
  - "Review follow-through frontend full/build gates: cd dashboard; npm test -- --run -> 25 files, 157 tests passed; cd dashboard; npm run build -> passed."
  - "Review follow-through Playwright smoke against Vite with mocked local API -> after Latest action is empty with one handled latest card and one older open high card, real React state switches the toolbar to High (1) and the main Review card is Old High Backlog. Screenshot written under ignored output/playwright/inbox-followthrough-high-desktop.png; Vite server stopped after smoke."
  - "Review follow-through diff gate: git diff --check -> passed."
  - "Notification target targeted frontend gate: cd dashboard; npm test -- --run settings -> 4 files, 21 tests passed."
