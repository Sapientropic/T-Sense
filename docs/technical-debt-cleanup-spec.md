...
| D3. `dashboard_server.py` boundaries | Artifact, git, fixed dry-run scheduler, Bot Gateway background, credentials facade, Telegram login, delivery settings, secret settings, source registry, source access, source assistant, action execution, profile creation, server selection, HTTP security, profile route mutation helpers, state payload assembly, GET route dispatch, and settings POST routes are split behind the old facade. The facade is currently `1047` lines and mainly owns the remaining POST route dispatch groups, pre-state-access guards, static serving, exception mapping, and compatibility re-exports. | Only split POST route groups when they have coherent focused tests and can preserve old monkeypatch paths; avoid low-value line shaving. |
...
| --- | ---: | ---: | --- |
| Python server | `scripts/dashboard_server.py` | 1047 | Remaining POST routing, static serving, route-level validation, pre-state-access guards, exception mapping, and compatibility re-exports remain in the facade after GET dispatch, state payload assembly, and settings POST dispatch moved out. |
