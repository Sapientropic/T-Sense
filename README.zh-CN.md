...
哪些消息值得看，再把结果放进本地工作流里处理。现在的核心界面是
**Signal Desk**：一个跑在本机浏览器里的 dashboard，用来完成 setup、扫描、
Review、profile 调整、运行健康检查和设置。
...
Profile 是一份 Markdown 规则，描述什么算信号、什么应该排除、偏好什么语言和来源、
报告里要如何呈现。
...
内置 starter 关注开发者机会；模板里还包括市场/新闻、空投、研究线索、竞品监控等方向。

## 报告

每次扫描都可以生成一个自包含 HTML 报告，包含卡片、来源链接、判断标签、诊断信息和运行元数据。
